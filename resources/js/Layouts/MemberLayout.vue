...
const navItems = [
    { name: 'Dashboard', href: '/member/dashboard', icon: 'grid_view', match: '/member/dashboard' },
    { name: 'Progress', href: '/member/progress', icon: 'trending_up', match: '/member/progress' },
    { name: 'Laporan', href: '/member/report', icon: 'assessment', match: '/member/report' },
]
...
watch(() => flash.value, (f) => {
    if (f?.success) { successMessage.value = f.success; showSuccess.value = true }
    if (f?.error) { errorMessage.value = f.error; showError.value = true }
}, { immediate: true })

const closeUserMenu = () => { userMenuOpen.value = false }
const logout = () => { userMenuOpen.value = false; router.post('/logout') }
</script>
...
<template>
    <div class="flex h-dvh overflow-hidden" @click="closeUserMenu">
        ...
        >
            <div v-if="sidebarOpen" class="fixed inset-0 bg-black/40 backdrop-blur-sm z-40 lg:hidden" @click="closeSidebar" />
        </Transition>
        ...
        >
            ...
            <div class="p-5 flex items-center gap-3 border-b border-slate-100">
                ...
                <div>
                    ...
                    <p class="text-[10px] text-slate-400 mt-0.5 uppercase tracking-wider font-semibold">Member Portal</p>
                </div>
            ...
            <nav class="flex-1 overflow-y-auto p-4 space-y-1">
                ...
                <Link
                    v-for="item in navItems" :key="item.name" :href="item.href" preserve-scroll
                    :class="[
                        isActive(item.match) ? 'bg-blue-50 text-blue-600 font-semibold' : 'text-slate-600 hover:bg-slate-50',
                        'flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200'
                    ]"
                    @click="closeSidebar"
                >
                    <span class="material-symbols-rounded text-[20px]" :class="isActive(item.match) ? 'text-blue-600' : 'text-slate-400'">{{ item.icon }}</span>
            ...
            <div class="p-4 border-t border-slate-100">
                <div class="flex items-center gap-3 p-2 rounded-xl hover:bg-slate-50 transition-colors cursor-pointer" @click.stop="userMenuOpen = !userMenuOpen">
                    <div class="w-9 h-9 rounded-lg bg-blue-50 border border-blue-100 flex items-center justify-center text-blue-600 text-xs font-bold">{{ userInitials }}</div>
                    <div class="flex-1 min-w-0">
                        <h4 class="text-sm font-semibold truncate">{{ userName }}</h4>
                        <p class="text-[11px] text-slate-400 truncate">Member</p>
                    </div>
                    <span class="material-symbols-rounded text-slate-400 text-[18px]">unfold_more</span>
                </div>
                <Transition enter-active-class="transition duration-200" enter-from-class="opacity-0 -translate-y-2" enter-to-class="opacity-100 translate-y-0" leave-active-class="transition duration-150" leave-from-class="opacity-100" leave-to-class="opacity-0 -translate-y-2">
        ...
        <main class="flex-1 flex flex-col overflow-hidden">
            ...
            <header class="lg:hidden flex items-center justify-between px-4 py-3 bg-white border-b border-slate-200 sticky top-0 z-30">
                ...
                <div class="w-8 h-8 rounded-lg bg-blue-50 border border-blue-100 flex items-center justify-center text-blue-600 text-[10px] font-bold">{{ userInitials }}</div>
            </header>

            <!-- Page Content -->
            <div class="flex-1 overflow-y-auto">
                <div class="mx-4 lg:mx-6 mt-4 space-y-2">
                    <Toast :message="successMessage" type="success" :show="showSuccess" @close="showSuccess = false" />
                    <Toast :message="errorMessage" type="error" :show="showError" @close="showError = false" />
                </div>
